...
public class PullLimsFacilityWideLabResultsTask extends AbstractTask {
	...
	public void execute() {
		...
		if (debugMode) System.out.println("PULL Lims Lab Results and persist to EMR: PULL TASK Starting");
		Context.openSession();
		...
		try {
			...
			try {
				...
				if (limsIntegrationEnabled.equals("false")) {
					...
					if (debugMode) System.out.println("Fetch Datetime ==>"+midnightDateTime);

					KenyaemrOrdersService kenyaemrOrdersService = Context.getService(KenyaemrOrdersService.class);
					List<LimsQueue> testsWithPendingResults = kenyaemrOrdersService.getLimsSubmittedEntriesByStatus(LimsQueueStatus.SUBMITTED, midnightDateTime, null, false);
					List<Integer> activeTestOrders = new ArrayList<>();
					if (debugMode) System.out.println("Lab tests pending results from LIMS: " + testsWithPendingResults.size());
					if (!testsWithPendingResults.isEmpty()) {
						if (debugMode) System.out.println("Found tests with pending results");
						for (LimsQueue queueEntry : testsWithPendingResults) {
							activeTestOrders.add(queueEntry.getOrder().getOrderId());
						}
						try {
							if (debugMode) System.out.println("Pull Lab Results and persist");
						...
						if (debugMode) System.out.println("LIMS-EMR integration: there are no lab tests with pending results");
						return;
					}
				}
			...
			try {
				...
				if (debugMode) System.out.println("Lims Lab Results PULL: Failed to check internet connectivity" + e.getMessage());
